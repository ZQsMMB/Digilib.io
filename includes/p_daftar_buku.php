<?php
	...
	$db->execute("SELECT b.kodeBuku, b.judul, p1.nama, p2.nama, b.tahun, b.edisi, b.issn_isbn, 
		...
		limit $posisi, $batas"); //line ini ditambahkan b.image (PPT Page 5)
	$data = $db->get_dataset();
	if(count($data) > 0) {
?>
<div class="row-fluid">
...
<?php	
	...
	$db->execute("SELECT b.kodeBuku, b.judul, p1.nama, p2.nama, b.tahun, b.edisi, b.issn_isbn, 
		...
		AND b.kodePengarang = p2.kodePengarang
		AND b.kodeKategori = k.kodeKategori
		AND pt.kodePetugas = b.lastUpdateBy"); //line ini ditambahkan b.image
...
<div class="modal hide fade" id="modalDetail<?=$i?>" tabindex="-1" role="dialog" aria-labelledby="modalDetail<?=$i?>Label" aria-hidden="true">
  ...
  <div class="modal-body">
      <div class="row-fluid">
		<div class="span4">
			<p><img src="<?=$data [$i][12]?>" class="img-polaroid img"></p> <!--PPT Page 3 dan 5-->
		</div>
...
<!-- EDIT -->
<?php for($i = 0; $i < count($data); $i++) { ?>
<form action="?page=process" method="post" enctype="multipart/form-data"> <!--PPT Page 3-->
      ...
      <div class="row-fluid">
		...
		<div>
			...
			<label><p class="title">Gambar</p>
				<input class="btn btn-success" type="file" name="image" accept=".jpg, .jpeg, .png"><!-- Menambahkan format image selain jpg-->
			</label>
...
<div class="modal hide fade" id="modalTambah" tabindex="-1" role="dialog" aria-labelledby="modalTambahLabel" aria-hidden="true">
  ...
  <div class="modal-body">
      <div class="row-fluid">
		...
		<div>
			<label><p class="title">Judul Buku</p>
			  <input name="judul" type="text" class="span12" value="" required="required"> <!--Menambahkan Required (PPT PAGE 6)-->
			</label>
			<label><p class="title">Gambar</p>
				<input class="btn btn-success" type="file" name="image" required="required"> <!--Menambahkan Required (PPT PAGE 6)-->
			</label>
			<label><p class="title">Penerbit</p>
				<select name="kodePenerbit" class="span12" required="required"> <!--Menambahkan Required (PPT PAGE 6)-->
				  <?php
					$db=new MySQL();
					$db->connect();
					$db->execute("SELECT kodePenerbit, nama FROM tb_penerbit"); 
